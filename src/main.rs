...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("--- Paste this before the </registry> closer ---");
    println!();
    for w in &wranglings {
        println!(
            "<!-- META EXT - Generated By: {} -->",
...
fn handle_typedef(w: &mut Wranglings, e: Entity<'_>) {
    ...
    if let TypeKind::Pointer = ty.get_kind() {
        ...
        if let Some(EntityKind::StructDecl) = children.first().map(Entity::get_kind) {
        ...
        } else if let Some(EntityKind::TypeRef) = children.first().map(Entity::get_kind) {
            let name = e.get_display_name().unwrap();
            let def = children.first().unwrap().get_display_name().unwrap();
            if def == "XrFlags64" {
                let vendor_index = name.rfind(char::is_lowercase).unwrap() + 1;
                let vendor = &name[vendor_index..];
                let flag_bits_name = format!(
                    "{}FlagBits{vendor}",
                    &name[..vendor_index].trim_end_matches("Flags")
                );
                writeln!(
                    w.types_xml,
                    r#"<type bitvalues="{flag_bits_name}" category="bitmask">typedef <type>XrFlags64</type> <name>{name}</name>;</type>"#
                );
                writeln!(
                    w.types_xml,
                    r#"<type name="{flag_bits_name}" category="enum"/>"#
                );
                writeln!(
                    w.extra_xml,
                    r#"<enums name="{flag_bits_name}" type="bitmask">"#
                );
                // TODO: variants. they usually come after a comment "// Flag bits for {name}"
                writeln!(w.extra_xml, r#"</enums>"#);

                w.types.push(name);
                w.types.push(flag_bits_name);
            }
        }
    }
}
